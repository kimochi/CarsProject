	...
	public function edit($id = null) {
		...
		if ($this->request->is(array('post', 'put'))) {
			if ($this->Type->save($this->request->data)) {
				...
				$file = $this->request->data['Type']['image_file'];
				//s'il en existe
				if (isset($file)) {
					...
					if (!empty($file['tmp_name'])) {
						$oldextension = strtolower(pathinfo($this->Type->field['image'], PATHINFO_EXTENSION)); //extension l9dima au cas ou bgha ymodifi l'image ila déja kayna 
						$oldfile = IMAGES  . 'uploads\types' . DS. $this->Type->field['image'];//nchdo l'image (objét)
						if (file_exists($oldfile)) {
							//supprimer si une existe déja
							unlink($oldfile);
						}
						//et aprés enregistrer l'image
					move_uploaded_file($file['tmp_name'], IMAGES .'uploads\types'. DS .'_' .$this->Type->id .'.' . $extension );
					//enregistrer le nom de l'image dans la base de données
					$this->Type->saveField('image','_' .$this->Type->id . '.' . $extension);
					}
				}
				$this->Session->setFlash(__('The type has been saved.'));
				return $this->redirect(array('action' => 'index'));
